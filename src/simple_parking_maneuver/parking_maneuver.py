...
BACKWARD_ARC_DISTANCE = 0.11


def callbackDrivingControl(msg):
    last_driving_control_info = msg.data

def multi_back(driving_direction_pub, publisher_forward, steer_distance,forward_distance, repetitions):
    assert repetitions > 0
    for time in range(repetitions-1):
        rospy.sleep(SLEEP_INTERVAL)
        driving_direction_pub.publish(steer_distance)
        rospy.sleep(SLEEP_INTERVAL)
        publisher_forward.publish(forward_distance)
    rospy.sleep(SLEEP_INTERVAL)
    driving_direction_pub.publish(steer_distance)
...
def callbackBackwardLongitudinal(request):
    ...
    multi_back(driving_direction_pub1, pub_forward, BACKWARD_ARC_DISTANCE,FORWARD_DISTANCE, 1)

    rospy.sleep(SLEEP_INTERVAL)
    pub_back.publish(0.06)

    multi_back(driving_direction_pub2, pub_forward, BACKWARD_ARC_DISTANCE,FORWARD_DISTANCE, 1)
    rospy.sleep(SLEEP_INTERVAL)    
